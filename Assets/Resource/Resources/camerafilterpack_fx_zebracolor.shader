Shader "CameraFilterPack/FX_ZebraColor" {
	Properties {
		_MainTex ("Base (RGB)", 2D) = "white" {}
		_TimeX ("Time", Range(0, 1)) = 1
		_Distortion ("_Distortion", Range(0, 1)) = 0.3
		_ScreenResolution ("_ScreenResolution", Vector) = (0,0,0,0)
		_Value ("_Value", Range(1, 10)) = 10
	}
	SubShader {
		Pass {
			ZClip Off
			ZTest Always
			GpuProgramID 62590
			Program "vp" {
				SubProgram "d3d9 " {
					"vs_3_0
					
					//
					// Generated by Microsoft (R) HLSL Shader Compiler 10.1
					//
					// Parameters:
					//
					//   row_major float4x4 glstate_matrix_mvp;
					//
					//
					// Registers:
					//
					//   Name               Reg   Size
					//   ------------------ ----- ----
					//   glstate_matrix_mvp c0       4
					//
					
					    vs_3_0
					    dcl_position v0
					    dcl_color v1
					    dcl_texcoord v2
					    dcl_texcoord o0.xy
					    dcl_position o1
					    dcl_color o2
					    dp4 r0.x, c0, v0
					    dp4 r0.y, c1, v0
					    dp4 r0.z, c2, v0
					    dp4 r0.w, c3, v0
					    mov o0.xy, v2
					    mov o2, v1
					    mad o1.xy, r0.w, c255, r0
					    mov o1.zw, r0
					
					// approximately 8 instruction slots used"
				}
				SubProgram "d3d11 " {
					"vs_4_0
					
					#version 330
					#extension GL_ARB_explicit_attrib_location : require
					#extension GL_ARB_explicit_uniform_location : require
					
					#define HLSLCC_ENABLE_UNIFORM_BUFFERS 1
					#if HLSLCC_ENABLE_UNIFORM_BUFFERS
					#define UNITY_UNIFORM
					#else
					#define UNITY_UNIFORM uniform
					#endif
					#define UNITY_SUPPORTS_UNIFORM_LOCATION 1
					#if UNITY_SUPPORTS_UNIFORM_LOCATION
					#define UNITY_LOCATION(x) layout(location = x)
					#define UNITY_BINDING(x) layout(binding = x, std140)
					#else
					#define UNITY_LOCATION(x)
					#define UNITY_BINDING(x) layout(std140)
					#endif
					layout(std140) uniform UnityPerDraw {
						mat4x4 glstate_matrix_mvp;
						vec4 unused_0_1[18];
					};
					in  vec4 in_POSITION0;
					in  vec4 in_COLOR0;
					in  vec2 in_TEXCOORD0;
					out vec2 vs_TEXCOORD0;
					out vec4 vs_COLOR0;
					vec4 u_xlat0;
					void main()
					{
					    vs_TEXCOORD0.xy = in_TEXCOORD0.xy;
					    u_xlat0 = in_POSITION0.yyyy * glstate_matrix_mvp[1];
					    u_xlat0 = glstate_matrix_mvp[0] * in_POSITION0.xxxx + u_xlat0;
					    u_xlat0 = glstate_matrix_mvp[2] * in_POSITION0.zzzz + u_xlat0;
					    gl_Position = glstate_matrix_mvp[3] * in_POSITION0.wwww + u_xlat0;
					    vs_COLOR0 = in_COLOR0;
					    return;
					}"
				}
			}
			Program "fp" {
				SubProgram "d3d9 " {
					"ps_3_0
					
					//
					// Generated by Microsoft (R) HLSL Shader Compiler 10.1
					//
					// Parameters:
					//
					//   sampler2D _MainTex;
					//   float4 _ScreenResolution;
					//   float _Value;
					//
					//
					// Registers:
					//
					//   Name              Reg   Size
					//   ----------------- ----- ----
					//   _ScreenResolution c0       1
					//   _Value            c1       1
					//   _MainTex          s0       1
					//
					
					    ps_3_0
					    def c2, 0.0208350997, -0.0851330012, 0.180141002, -0.330299497
					    def c3, 0.999866009, 0, 1, 3.14159274
					    def c4, -2, 1.57079637, 0.636619747, 0.333333343
					    def c5, 0.159154937, 0.5, 6.28318548, -3.14159274
					    dcl_texcoord v0.xy
					    dcl_2d s0
					    texld r0, v0, s0
					    add r1.xy, -r0_abs.yxzw, r0_abs
					    cmp r1.xz, r1.x, r0_abs.yyxw, r0_abs.xyyw
					    cmp r0.w, r1.y, c3.y, c3.z
					    rcp r1.y, r1.z
					    mul r1.x, r1.y, r1.x
					    mul r1.y, r1.x, r1.x
					    mad r1.z, r1.y, c2.x, c2.y
					    mad r1.z, r1.y, r1.z, c2.z
					    mad r1.z, r1.y, r1.z, c2.w
					    mad r1.y, r1.y, r1.z, c3.x
					    mul r1.x, r1.y, r1.x
					    mad r1.y, r1.x, c4.x, c4.y
					    mad r0.w, r1.y, r0.w, r1.x
					    cmp r1.x, r0.y, -c3.y, -c3.w
					    add r0.w, r0.w, r1.x
					    add r1.x, r0.w, r0.w
					    add r1.y, -r0.y, r0.x
					    cmp r1.yz, r1.y, r0.xyxw, r0.xxyw
					    cmp r1.z, r1.z, c3.z, c3.y
					    cmp r1.y, r1.y, c3.y, r1.z
					    mad r0.w, r1.y, -r1.x, r0.w
					    mul r0.w, r0.w, c1.x
					    mul r0.w, r0.w, c4.z
					    frc r1.x, r0.w
					    add r0.w, r0.w, -r1.x
					    mul r0.w, r0.w, c3.w
					    rcp r1.x, c1.x
					    mad r0.w, r0.w, r1.x, c4.y
					    mad r0.w, r0.w, c5.x, c5.y
					    frc r0.w, r0.w
					    mad r0.w, r0.w, c5.z, c5.w
					    sincos r2.xy, r0.w
					    mul r1.yz, c0.xxyw, v0.xxyw
					    dp2add r0.w, r2, r1.yzzw, c3.y
					    add r1.y, r0.y, r0.x
					    add r1.y, r0.z, r1.y
					    nrm r2.xyz, r0
					    mul r0.x, r1.y, c1.x
					    mul r0.x, r0.x, c4.w
					    frc r0.y, r0.x
					    add r0.x, -r0.y, r0.x
					    mul r0.x, r1.x, r0.x
					    mul r0.x, r0.w, r0.x
					    mad r0.x, r0.x, c5.y, c5.y
					    frc r0.x, r0.x
					    mad r0.x, r0.x, c5.z, c5.w
					    sincos r1.x, r0.x
					    mad r0.x, r1.x, -c5.y, c5.y
					    mul oC0.xyz, r2, r0.x
					    mov oC0.w, c3.z
					
					// approximately 68 instruction slots used (1 texture, 67 arithmetic)"
				}
				SubProgram "d3d11 " {
					"ps_4_0
					
					#version 330
					#extension GL_ARB_explicit_attrib_location : require
					#extension GL_ARB_explicit_uniform_location : require
					
					#define HLSLCC_ENABLE_UNIFORM_BUFFERS 1
					#if HLSLCC_ENABLE_UNIFORM_BUFFERS
					#define UNITY_UNIFORM
					#else
					#define UNITY_UNIFORM uniform
					#endif
					#define UNITY_SUPPORTS_UNIFORM_LOCATION 1
					#if UNITY_SUPPORTS_UNIFORM_LOCATION
					#define UNITY_LOCATION(x) layout(location = x)
					#define UNITY_BINDING(x) layout(binding = x, std140)
					#else
					#define UNITY_LOCATION(x)
					#define UNITY_BINDING(x) layout(std140)
					#endif
					layout(std140) uniform PGlobals {
						vec4 unused_0_0[3];
						vec4 _ScreenResolution;
						float _Value;
					};
					uniform  sampler2D _MainTex;
					in  vec2 vs_TEXCOORD0;
					layout(location = 0) out vec4 SV_Target0;
					vec4 u_xlat0;
					vec2 u_xlat1;
					bool u_xlatb1;
					vec2 u_xlat2;
					float u_xlat4;
					bool u_xlatb4;
					bool u_xlatb7;
					float u_xlat9;
					void main()
					{
					    u_xlat0 = texture(_MainTex, vs_TEXCOORD0.xy);
					    u_xlat9 = max(abs(u_xlat0.y), abs(u_xlat0.x));
					    u_xlat9 = float(1.0) / u_xlat9;
					    u_xlat1.x = min(abs(u_xlat0.y), abs(u_xlat0.x));
					    u_xlat9 = u_xlat9 * u_xlat1.x;
					    u_xlat1.x = u_xlat9 * u_xlat9;
					    u_xlat4 = u_xlat1.x * 0.0208350997 + -0.0851330012;
					    u_xlat4 = u_xlat1.x * u_xlat4 + 0.180141002;
					    u_xlat4 = u_xlat1.x * u_xlat4 + -0.330299497;
					    u_xlat1.x = u_xlat1.x * u_xlat4 + 0.999866009;
					    u_xlat4 = u_xlat9 * u_xlat1.x;
					    u_xlat4 = u_xlat4 * -2.0 + 1.57079637;
					    u_xlatb7 = abs(u_xlat0.y)<abs(u_xlat0.x);
					    u_xlat4 = u_xlatb7 ? u_xlat4 : float(0.0);
					    u_xlat9 = u_xlat9 * u_xlat1.x + u_xlat4;
					    u_xlatb1 = u_xlat0.y<(-u_xlat0.y);
					    u_xlat1.x = u_xlatb1 ? -3.14159274 : float(0.0);
					    u_xlat9 = u_xlat9 + u_xlat1.x;
					    u_xlat1.x = min(u_xlat0.y, u_xlat0.x);
					    u_xlatb1 = u_xlat1.x<(-u_xlat1.x);
					    u_xlat4 = max(u_xlat0.y, u_xlat0.x);
					    u_xlatb4 = u_xlat4>=(-u_xlat4);
					    u_xlatb1 = u_xlatb4 && u_xlatb1;
					    u_xlat9 = (u_xlatb1) ? (-u_xlat9) : u_xlat9;
					    u_xlat9 = u_xlat9 * _Value;
					    u_xlat9 = u_xlat9 * 0.636619747;
					    u_xlat9 = floor(u_xlat9);
					    u_xlat9 = u_xlat9 * 3.14159274;
					    u_xlat9 = u_xlat9 / _Value;
					    u_xlat9 = u_xlat9 + 1.57079637;
					    u_xlat1.x = sin(u_xlat9);
					    u_xlat2.x = cos(u_xlat9);
					    u_xlat2.y = u_xlat1.x;
					    u_xlat1.xy = vs_TEXCOORD0.xy * _ScreenResolution.xy;
					    u_xlat9 = dot(u_xlat2.xy, u_xlat1.xy);
					    u_xlat1.x = u_xlat0.y + u_xlat0.x;
					    u_xlat1.x = u_xlat0.z + u_xlat1.x;
					    u_xlat1.x = u_xlat1.x * _Value;
					    u_xlat1.x = u_xlat1.x * 0.333333343;
					    u_xlat1.x = floor(u_xlat1.x);
					    u_xlat1.x = u_xlat1.x / _Value;
					    u_xlat9 = u_xlat9 * u_xlat1.x;
					    u_xlat9 = u_xlat9 * 3.14159274;
					    u_xlat9 = cos(u_xlat9);
					    u_xlat9 = (-u_xlat9) * 0.5 + 0.5;
					    u_xlat1.x = dot(u_xlat0.xyz, u_xlat0.xyz);
					    u_xlat1.x = sqrt(u_xlat1.x);
					    u_xlat0.xyz = u_xlat0.xyz / u_xlat1.xxx;
					    SV_Target0.xyz = u_xlat0.xyz * vec3(u_xlat9);
					    SV_Target0.w = 1.0;
					    return;
					}"
				}
			}
		}
	}
}